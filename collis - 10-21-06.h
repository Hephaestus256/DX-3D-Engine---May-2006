	...
	prim_pool* pp, poly_type* p, point_3d& c_t, point_3d& t_t, float rad, float h, point_3d& r)
{
	...
	if (top (pt)) {
		hit = true;		
		pt = cut_path_tb (pt);
		set_closer (pt, curr_t, closest);
	}
	if (bot (pt)) {
		hit = true;
		pt = offset_3d (pt, 0.0f, height, 0.0f);
		pt = cut_path_tb (pt);
		set_closer (pt, curr_t, closest);
	}
...
/*
	if (front (pt)) {
		...
		//pt = cut_path_f (pt);
//		set_closer (pt, curr_t, closest);
		//r = point_3d (10.0f, -5.0f, -6.0f);//curr_t;
		r = pt;
	}
*/
//	r = calc_new_loc (closest);
	//r = closest;
	if (left (pt)) {
		r = calc_new_loc (pt);
		hit = true;
...
bool collis::top (point_3d& res)
{
	...
	poly_int_hex_solid (vert, poly->verts, top_hex, intr, ct);
	
//	for (int i = 0; i < ct; i++)
//		create_test_point (intr[i], i, RGB_BLUE);

	return false;
	//if (get_path (path)) {
		//check_shield (perp_xz + 0.0, path, res, hit);
	//}

/*
	if (plane.mx == PLANE_Y_PLANE) {
		line_3d path (curr_t, tent_t);
		line_3d_int_plane (path, plane, res);

		if (point_in_poly (res, vert, poly->verts, dum) == -1)
			if (side)
				if (point_rel_plane (tent_t, plane) == 1)
					return true;
		...
		bool imp[4];
		point_3d path[2];
		point_3d path_sorted[2];
		point_3d impact[4];
		int path_ct = 0;
		int dir = trin_cmp (tent_t.y, curr_t.y);

		imp[0] = seg_int_plane_inc (curr_ot, curr_nt, plane, impact[0]); 
		imp[1] = seg_int_plane_inc (tent_ot, tent_nt, plane, impact[1]);
		imp[2] = seg_int_plane_exc (curr_ot, tent_ot, plane, impact[2]);
		imp[3] = seg_int_plane_exc (curr_nt, tent_nt, plane, impact[3]);

		for (int i = 0; i < 4; i++)
			if (imp[i]) 
				path[path_ct++] = impact[i];

		sort_path_y (path, path_sorted, dir);

		if (point_rel_plane (curr_ot, plane) == 0)
			if (point_rel_plane (tent_ot, plane) == 0)
				return false;

		if (path_ct == 2) 
			//return area_touch_y (path, path_sorted, res, dir);		
			return area_touch (path_sorted, res);
...
bool collis::bot (point_3d& res)
{
	...
	poly_int_hex_solid (vert, poly->verts, bot_hex, intr, ct);
...
/*
	if (plane.mx == PLANE_Y_PLANE) {
		line_3d path (curr_b, tent_b);
		line_3d_int_plane (path, plane, res);

		if (point_in_poly (res, vert, poly->verts, dum) == -1)
			if (!side)
				if (point_rel_plane (tent_b, plane) == -1)
					return true;
		...
		point_3d impact[4];
		int path_ct = 0;
		int dir = trin_cmp (tent_b.y, curr_b.y);
		int i;

		imp[0] = seg_int_plane_inc (curr_ob, curr_nb, plane, impact[0]); 
		imp[1] = seg_int_plane_inc (tent_ob, tent_nb, plane, impact[1]);
		imp[2] = seg_int_plane_exc (curr_ob, tent_ob, plane, impact[2]);
		imp[3] = seg_int_plane_exc (curr_nb, tent_nb, plane, impact[3]);
		...
		if (point_rel_plane (curr_ob, plane) == 0)
			if (point_rel_plane (tent_ob, plane) == 0)
				return false;
...
bool collis::left (point_3d& res)
{
	...
	double m = calc_dzdx_m (rig_hex[0], rig_hex[1]);
	//dprint ("mm ", m, 17);
	res = closest_hit (m, intr, ct, path, sum (d_right, ORIG));
	
	//create_test_point (res, 66, RGB_BLUE);
	//create_test_point (rig_hex[0], 66, RGB_BLUE);
	//create_test_point (rig_hex[1], 67, RGB_BLUE);

	//for (int i = 0; i < ct; i++)
	//	create_test_point (intr[i], i, RGB_BLUE);
	if (h) 
		print_point ("res ", res, 17);
...
bool collis::right (point_3d& res)
{
	...
	poly_int_hex_solid (vert, poly->verts, rig_hex, intr, ct);
...
/*
	create_shield_paths (0.0);
	point_3d path[2];
//	bool hit;
	point_3d hex_solid[8];
	point_3d intr[256];
	int ct;

	create_shield_path (PI / 2.0 + 0.0, hex_solid);
	poly_int_hex_solid (vert, poly->verts, hex_solid, intr, ct);
	
	//for (int i = 0; i < ct; i++)
		//create_test_point (intr[i], i + 25, RGB_GREEN);
	//if (get_path (path)) {
		//check_shield (perp_xz + 0.0, path, res, hit);
	//}
	...
	if (approx_zero (plane.my)) {
		...
		//if (seg_int_plane_exc (curr_ot, tent_ot, plane, pen))
		//	imp = true;
		//if (point_rel_plane (curr_ot, plane) == 0)
		//	imp = 
		//point_in_plane (
		if (seg_int_plane_inc (curr_ot, tent_ot, plane, pen))
			if (side) {
				if (point_rel_plane (tent_ot, plane) == 1)
					imp = true;
			}
			else {
				if (point_rel_plane (tent_ot, plane) == -1)
		...
		if (imp) {
			...
			point_3d path[2];
			path[0] = pen;
			path[1] = offset_3d (pen, 0.0f, -height, 0.0f);
			
			res = pen;
			//return area_touch_y (path, path, dum, -1);
			return area_touch (path, res);
//////////
//			impact[0] = sum (d_right, pen);
//			impact[1] = diff (d_right, pen);
//			impact[2] = offset_3d (impact[0], 0.0f, -height, 0.0f);
//			impact[3] = offset_3d (impact[1], 0.0f, -height, 0.0f);
//			
//			for (i = 0; i < poly->verts; i++)
//				if (point_in_poly (vert[i], impact, 4, dum) == -1)
//					hit = true;
//
//			for (i = 0; i < 4; i++)
//				if (point_in_poly (impact[i], vert, poly->verts, dum) == -1)
//					hit = true;
//
//			res = pen;
//			return hit;
////// 
		...
		//double cb = double(curr_t.z) - plane.mx * double(curr_t.x);
		//double tb = double(tent_t.z) - plane.mx * double(tent_t.x);

		imp[0] = seg_int_plane_inc (curr_ot, curr_ob, plane, impact[0]); 
		imp[1] = seg_int_plane_inc (tent_ot, tent_ob, plane, impact[1]);
		imp[2] = seg_int_plane_exc (curr_ot, tent_ot, plane, impact[2]);
		imp[3] = seg_int_plane_exc (curr_ob, tent_ob, plane, impact[3]);
		...
		//if (point_rel_plane (tent_ot, plane) == 0)
		//		return false;
		sort_path_xz (path, path_sorted);

		//int dir = trin_cmp (path_sorted[1].y, path_sorted[0].y);
...
//		}
		if (path_ct == 2) {
//			print_point ("0 ", path[0], 9);
//			print_point ("1 ", path[1], 10);
		}
...
//point_3d collis::get_closest (point_3d* imps, int imp_ct)
//{
