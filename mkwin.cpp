...
WindowCreator::WindowCreator(char const * caption, char const * className, HINSTANCE hInstance) {
	_hwnd = ::CreateWindow(
		...
		1000,//CW_USEDEFAULT,	// width of the window
		500,//CW_USEDEFAULT,	// height of the window
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, char * cmdParam, int cmdShow) {
	...
	WindowCreator window("A useless window", className, hInst);
	window.SetVisible(cmdShow);	
	
	MessageBox(window._hwnd, "Failed to initialize DirectDraw", "Error", MB_OK );
	MessageBox(window._hwnd, "another", "Crap!!!", MB_OK );
