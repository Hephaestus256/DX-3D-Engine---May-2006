...
class collis {
	...
	point_3d closest_hit (double m, point_3d* intersect, int ict, point_3d* path);
...
point_3d collis::closest_hit (double m, point_3d* intersect, int ict, point_3d* path)
{
	...
	if (approx_equal (b0, b1)) {
		double m_dxdz = 0;// = calc_dxdz_m (e1, e2);
		exit(1);
		double min_b = calc_dxdz_b (intersect[0], m_dxdz);
		point_3d min = intersect[0];

		for (int i = 0; i < ict; i++) {
			double b = calc_dxdz_b (intersect[i], m_dxdz);
		...
		double min_b = calc_dzdx_b (intersect[0], m);
		point_3d min = intersect[0];

		for (int i = 0; i < ict; i++) {
			double b = calc_dzdx_b (intersect[i], m);
		...
		double max_b = calc_dzdx_b (intersect[0], m);
...
bool collis::get_path (point_3d* path)
{
	...
	hit[0] = seg_int_plane_inc (curr_t, tent_t, plane, p[0]);
	hit[1] = seg_int_plane_inc (curr_b, tent_b, plane, p[1]);
	hit[2] = seg_int_plane_exc (curr_t, curr_b, plane, p[2]);
	hit[3] = seg_int_plane_exc (tent_t, tent_b, plane, p[3]);
...
void collis::check_shield (double da, point_3d* path, point_3d& r, bool& hit)
{
	int i;
	point_3d p;
